    ...
    <section id="catalogo" className="section" ref={sectionRef}>
      <div className="container">
        ...
        <div className="grid cards">
          {sucos.map((suco) => {
            const localizedName = suco.name?.[language] ?? suco.name?.en ?? suco.name?.pt ?? suco.name;
            const quantity = getJuiceQuantity?.(suco) ?? 0;

            return (
              <motion.article key={suco.id ?? localizedName} className="card" {...cardMotion}>
                <span className="card-slash" aria-hidden="true" />
                <div className="card-media">
                  <img className="card-bottle" src={suco.image} alt={localizedName} loading="lazy" />
                </div>
